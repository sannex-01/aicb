...
async def perform_sannex_sync(db: AsyncSession) -> Dict[str, Any]:
    ...
    sync_url = f"{settings.SANNEX_HOST.rstrip('/')}/v1/tenants/config"
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if res.status_code == 200:
                ...
                for key in ["system_prompt", "temperature", "model_name", "llm_provider", "bot_mode", "max_tokens"]:
                ...
                logger.warning(f"Sannex AgentOS sync returned status {res.status_code}: {res.text}")
...
def start_sync_scheduler():
    """Starts the 12-hour periodic background sync job."""
